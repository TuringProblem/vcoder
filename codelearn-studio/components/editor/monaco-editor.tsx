...
import { initVim, disposeVim } from "@/lib/vim-config"
import { Card } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
import { Wand2, Wrench, LocateFixed, ScanSearch } from "lucide-react"
import { useLspDiagnosticsSync, useHoverSync } from "@/hooks/use-lsp"
import { goToDefinition, findReferences, renameSymbolAtCursor } from "@/lib/lsp/typescript-lsp"
...
export function MonacoEditor() {
  ...
  const ensureModels = (monaco: typeof import("monaco-editor")) => {
    ...
    ;(Object.keys(MODEL_URIS) as Language[]).forEach((lang) => {
      if (!modelsRef.current[lang]) {
        // Try to reuse an existing model for this URI, otherwise create one
  ...
  // Switch model when language changes
  useEffect(() => {
  ...
  // Keep model in sync when "Templates" or "Load" mutate store without typing
  useEffect(() => {
    ...
    if (model && model.getValue() !== code) {
      // avoid moving cursor unexpectedly: push edit
  ...
  const monacoToolbar = (
    <div className="absolute right-3 top-3 z-10 flex gap-1">
      <div
        ...
        title={
          isLspOn ? "LSP features enabled (JS/TS)" : "Basic syntax only. Add an external LSP to enable IDE features."
        }
      >
