...
export function useCompleteLesson() {
  ...
  return useMutation<
    { language: LanguageKey; completedLessons: Record<string, boolean> },
    Error,
    CompleteLessonRequest
  >({
    ...
    onSuccess: (data) => {
      qc.invalidateQueries({
        queryKey: progressKey(data.language as LanguageKey),
      });
    },
  });
}
...
export function useAccessCheck(
  params: { language: LanguageKey; section: string; lesson: string },
  enabled = true,
) {
