...
export function CodeEditor({
  ...
  React.useEffect(() => {
    ...
    const enableVim = async () => {
      ...
      const mod: MonacoVimAPI = (await import(
        "monaco-vim"
      )) as unknown as MonacoVimAPI;
      if (cancelled) return;
      vimInstanceRef.current = new mod.VimMode(
        editor,
  ...
  return (
    <div
      className={
        "h-full w-full border rounded-md overflow-hidden " + (className ?? "")
      }
    >
      <Editor
        height="100%"
