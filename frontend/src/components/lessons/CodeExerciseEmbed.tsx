...
const MonacoEditor = React.lazy(() =>
  import("@/pages/dev-view/internal/editor/MonacoEditorImpl").then((m) => ({
    default: m.MonacoEditor,
  })),
);
const AnalysisPanel = React.lazy(() =>
  import("@/pages/dev-view/internal/analysis/AnalysisPanelImpl").then((m) => ({
...
export function CodeExerciseEmbed({
  context,
  initialCode,
  instructions,
}: CodeExerciseProps) {
  ...
  return (
    <div
      ...
      data-testid="code-exercise-embed"
    >
      ...
      <div>
        <React.Suspense
          fallback={
            <div className="p-4 text-sm text-muted-foreground">
              {t("editor.loadingAnalysis")}
            </div>
          }
        >
          <AnalysisPanel lessonContext={lessonContext} />
        </React.Suspense>
        <div className="mt-3 text-sm text-muted-foreground">{instructions}</div>
      </div>
    </div>
  );
}
