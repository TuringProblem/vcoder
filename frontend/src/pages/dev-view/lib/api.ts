...
function basicSyntaxScan(language: Language, code: string) {
  ...
  if (language === "javascript" || language === "typescript") {
    lines.forEach((ln, i) => {
      if (
        /^\s*(const|let|var|return|console\.log|if|for|while|function|interface|type|export|import)/.test(
          ln,
        )
      ) {
        if (
          !/[;{\s]$/.test(ln.trim()) &&
          !/^\s*(export|import)/.test(ln.trim())
        ) {
          add(
            i + 1,
            "info",
            "Consider consistent semicolons",
            "Enable a formatter or linter.",
          );
        }
      }
    });
    ...
    lines.forEach((ln, i) => {
      if (
        /^\s*(int|String|boolean|double|float|char|long|short|byte|System\.out\.println|return)\b/.test(
          ln,
        )
      ) {
        if (!/[;{\s]$/.test(ln.trim())) {
      ...
      if (/public\s+class\b/.test(ln) && !/\{/.test(ln)) {
        add(
          i + 1,
          "warning",
          "Class declaration without '{' on same line",
          "Open class block.",
        );
      }
    });
...
function qualityChecks(language: Language, code: string) {
  ...
  const loops = countOccurrences(code, [
    /for\s*\(/g,
    /while\s*\(/g,
    /for\s+\w+\s+in/g,
  ]);
  const conditionals = countOccurrences(code, [
    /if\s*\(/g,
    /\belif\b/g,
    /\belse\b/g,
    /\bswitch\b/g,
  ...
  if (
    ...
    /[A-Z]/.test(
      code
        .split(/\W+/)
        .find((t) => /^[a-zA-Z_]+$/.test(t) && t === t.toUpperCase()) || "",
    )
  ) {
    suggestions.push({
  ...
  const grade: "good" | "ok" | "poor" =
    score >= 80 ? "good" : score >= 60 ? "ok" : "poor";
...
function learningContent(language: Language, code: string) {
  ...
  if (language === "typescript") {
    topics.push({
      ...
      links: [
        {
          label: t("learning.topics.interfacesVsTypes.link"),
    ...
    topics.push({
      ...
      links: [
        {
          label: t("learning.topics.generics.link"),
  ...
  if (language === "python" && /range\(/.test(code)) {
    topics.push({
      ...
      links: [
        {
          label: t("learning.topics.forEachLoops.link"),
...
function performanceAnalysis(language: Language, code: string) {
  ...
  const bigO: "O(1)" | "O(n)" | "O(n^2)" =
    nestedLoops > 0 ? "O(n^2)" : loops > 0 ? "O(n)" : "O(1)";

  const hotspots: { title: string; line: number; detail: string }[] = [];

  if (nestedLoops > 0) {
    hotspots.push({
      title: t("performance.hotspots.nestedLoops.title"),
      line: 1,
      detail: t("performance.hotspots.nestedLoops.detail"),
    });
  }
...
function executionFlow(language: Language, code: string) {
  ...
  if (/main\s*\(/.test(code) || /def\s+main\s*\(/.test(code)) {
    ...
    steps.push({
      title: "Execute top-level",
      detail: "Run top-level statements.",
    });
  }
  if (/for\s*\(|while\s*\(|for\s+\w+\s+in/.test(code)) {
    steps.push({
      title: "Loop",
      detail: "Iterate over a sequence or condition.",
    });
  }
