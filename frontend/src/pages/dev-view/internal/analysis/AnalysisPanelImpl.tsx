...
import { useCodeAnalysis } from "@/pages/dev-view/hooks/use-code-analysis";
import { SyntaxTab } from "@/pages/dev-view/internal/analysis/SyntaxTab";
import { QualityTab } from "@/pages/dev-view/internal/analysis/QualityTab";
import { LearningTab } from "@/pages/dev-view/internal/analysis/LearningTab";
import { ExecutionTab } from "@/pages/dev-view/internal/analysis/ExecutionTab";
import { PerformanceTab } from "@/pages/dev-view/internal/analysis/PerformanceTab";
...
export function AnalysisPanel({
  ...
  const grade =
    (data?.quality?.grade as "good" | "ok" | "poor" | undefined) ??
    ("ok" as const);
  const gradeColor =
    grade === "good"
      ? "bg-green-500"
    ...
    <Card className="rounded-none md:rounded-md h-[calc(100vh-6rem)] md:h-[calc(100vh-8rem)] flex flex-col">
      <div className="flex items-center justify-between p-3 border-b">
        <div className="flex items-center gap-2">
          <div
            className={`h-2.5 w-2.5 rounded-full ${gradeColor}`}
            aria-hidden="true"
          />
          <span className="text-sm font-medium">
            {t("analysis.realTimeAnalysis")}
          </span>
          ...
          <div className="px-3 pt-3">
            <TabsList className="grid w-full grid-cols-5">
              <TabsTrigger value="syntax">
                {t("analysis.tabs.syntax")}
              </TabsTrigger>
              <TabsTrigger value="quality">
                {t("analysis.tabs.quality")}
              </TabsTrigger>
              <TabsTrigger value="execution">
                {t("analysis.tabs.execution")}
              </TabsTrigger>
              <TabsTrigger value="learning">
                {t("analysis.tabs.learning")}
              </TabsTrigger>
              <TabsTrigger value="performance">
                {t("analysis.tabs.performance")}
              </TabsTrigger>
            </TabsList>
          </div>
