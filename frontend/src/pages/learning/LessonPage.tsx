...
function AccessCheck({ isPractice, checking, access, onNavigate }: AccessCheckProps) {
  if (isPractice) return null
  
  if (checking) {
    return <div className="text-sm text-muted-foreground">{t("lessons.checkingAccess")}</div>
  }
  
  if (access && !access.allowed) {
    return (
      <>
        <div className="text-sm text-red-600" data-testid="lesson-locked-reason">
...
function EditorModeSelector({ isPractice, editorMode, onEditorModeChange }: EditorModeSelectorProps) {
  if (isPractice) return null
...
function LessonNavigation({ isPractice, language, section, lessonNumber, onNavigate, onComplete, isCompleting }: LessonNavigationProps) {
  if (isPractice) return null
  ...
  const handleNext = () => {
    ...
    if (nextLesson <= 9) {
      ...
      if (currentIndex < sections.length - 1) {
        onNavigate(`/course/${language}/${sections[currentIndex + 1]}/lesson-1`)
      }
    }
  }
  ...
  const handlePrevious = () => {
    ...
    if (prevLesson >= 1) {
      ...
      const currentIndex = sections.indexOf(section)
      if (currentIndex > 0) {
        onNavigate(`/course/${language}/${sections[currentIndex - 1]}/lesson-9`)
      }
    }
  }
